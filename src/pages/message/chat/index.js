	...
	getHistoryMsg = async()=> {
	  ...
	  // 处理数据
		...
		for(let i = 0; i < historyMsg.length; i++) {
			...
			obj["user"] = { 
				_id: this.props.UserStore.user.guid,
				name: this.props.UserStore.user.nick_name,
				avatar: BASE_URI+this.props.UserStore.user.header,
			}
  ...
  async onSend(messages = []) {
    this.setState(previousState => ({
      messages: GiftedChat.append(previousState.messages, messages),
		...
		console.log(`我在给${this.props.route.params.nick_name}发消息。。。。。。。。。。`);
